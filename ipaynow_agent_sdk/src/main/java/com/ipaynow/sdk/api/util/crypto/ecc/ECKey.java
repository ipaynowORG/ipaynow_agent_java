package com.ipaynow.sdk.api.util.crypto.ecc;
/**
 * Copyright 2011 Google Inc. Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file
...
import com.ipaynow.sdk.api.util.crypto.Constants;
import com.ipaynow.sdk.api.util.crypto.HashUtil;
import com.ipaynow.sdk.api.util.crypto.SpongyCastleProvider;
...
import static com.ipaynow.sdk.api.util.crypto.BIUtil.isLessThan;
import static com.ipaynow.sdk.api.util.crypto.ByteUtil.bigIntegerToBytes;
 ...
 * </p>
 * <p>
 * The ECDSA algorithm supports <i>key recovery</i> in which a signature plus a couple of discriminator bits can be
...
public class ECKey implements Serializable {
    ...
    /**
     * Equal to CURVE.getN().shiftRight(1), used for canonicalising the S value of a signature. ECDSA signatures are
     * mutable in the sense that for a given (R, S) pair, then both (R, S) and (R, N - S mod N) are valid signatures.
     * Canonical signatures are those where 1 <= S <= N/2 See
    ...
    /**
     * Generates an entirely new keypair with the given {@link SecureRandom} object. BouncyCastle will be used as the
    ...
    /*
     * Test if a generic private key is an EC private key it is not sufficient to check that privKey is a subtype of
     * ECPrivateKey as the SunPKCS11 Provider will return a generic PrivateKey instance a fallback that covers this case
     * is to check the key algorithm
     */
    ...
    public ECKey(Provider provider, @Nullable PrivateKey privKey, ECPoint pub){
        ...
        if (privKey == null || isECPrivateKey(privKey)) {
            ...
            throw new IllegalArgumentException("Expected EC private key, given a private key object with class "
                                               + privKey.getClass().toString() + " and algorithm "
    ...
    private static PrivateKey privateKeyFromBigInteger(BigInteger priv) {
        if (priv == null) {
            ...
            try {
                return ECKeyFactory.getInstance(SpongyCastleProvider.getInstance()).generatePrivate(new ECPrivateKeySpec(priv,
                                                                                                                         CURVE_SPEC));
    ...
    public static class ECDSASignature {
        ...
        public static ECDSASignature decodeFromDER(byte[] bytes) {
            ...
            try {
                ...
                // OpenSSL deviates from the DER spec by interpreting these values as unsigned, though they should not
                // be
        ...
        public byte[] toByteArray() {
            ...
            byte[] r_r = ByteUtil.bigIntegerToBytes(this.r, 32);
            byte[] r_s = ByteUtil.bigIntegerToBytes(this.s, 32);

            return ByteUtil.merge(r_r, r_s, new byte[] { fixedV });
        }
