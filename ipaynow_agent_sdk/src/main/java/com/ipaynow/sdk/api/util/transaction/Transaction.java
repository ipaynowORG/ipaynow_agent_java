package com.ipaynow.sdk.api.util.transaction;

import static com.ipaynow.sdk.api.util.crypto.ByteUtil.EMPTY_BYTE_ARRAY;
import static com.ipaynow.sdk.api.util.crypto.ByteUtil.ZERO_BYTE_ARRAY;
...
import com.ipaynow.sdk.api.util.crypto.ByteUtil;
import com.ipaynow.sdk.api.util.crypto.ecc.ECKey;
import com.ipaynow.sdk.api.util.crypto.HashUtil;
import com.ipaynow.sdk.api.util.crypto.rlp.RLP;
import com.ipaynow.sdk.api.util.crypto.rlp.RLPElement;
import com.ipaynow.sdk.api.util.crypto.rlp.RLPItem;
import com.ipaynow.sdk.api.util.crypto.rlp.RLPList;
...
public class Transaction {
     ...
     */
    private byte[]               gasPrice       = null;

    /*
     * the amount of "gas" to allow for the computation. Gas is the fuel of the computational engine; every
     * computational step taken and every byte added to the state or transaction list consumes some gas.
     */
    private byte[]               gasLimit;

    /*
     * An unlimited size byte array specifying input [data] of the message call or Initialization code for a new
     * contract
     */
    private byte[]               data;
    ...
    public Transaction(BigInteger nonce, String receiveAddress, BigInteger value, byte[] data) throws Exception{
        byte[] nonceBytes = EMPTY_BYTE_ARRAY;
        if (nonce != null) nonceBytes = ByteUtil.bigIntegerToBytes(nonce);
        this.nonce = nonceBytes;

        this.gasLimit = ByteUtil.bigIntegerToBytes(new BigInteger("4600000"));
        if (value == null) {
        ...
        if (StringUtils.isBlank(receiveAddress)) {
            ...
            byte[] addressBytes = TypeConverter.StringHexToByteArray(receiveAddress);
            this.receiveAddress = addressBytes;
        }
    ...
    private Integer extractChainIdFromV(BigInteger bv) {
        if (bv.bitLength() > 31) return Integer.MAX_VALUE; // chainId is limited to 31 bits, longer are not valid for
                                                           // now
        long v = bv.longValue();
        ...
        try {
            ...
            for (RLPElement rlpElement : transaction) {
                if (!(rlpElement instanceof RLPItem))
                    throw new RuntimeException("com.ipaynow.sdk.api.dto.Transaction RLP elements shouldn't be lists");
            }
    ...
    private void validate() {
        ...
        if (gasPrice != null && gasPrice.length > HASH_LENGTH) throw new RuntimeException("Gas Price is not valid");
        if (value != null && value.length > HASH_LENGTH) throw new RuntimeException("Value is not valid");
    ...
    public String toString(int maxDataSize) {
        ...
        return "TransactionData [" + "hash=" + ByteUtil.toHexString(hash) + "  nonce=" + ByteUtil.toHexString(nonce)
               + ", gasPrice=" + ByteUtil.toHexString(gasPrice) + ", gas=" + ByteUtil.toHexString(gasLimit)
               + ", receiveAddress=" + ByteUtil.toHexString(receiveAddress) + ", sendAddress="
               + ByteUtil.toHexString(getSender()) + ", value=" + ByteUtil.toHexString(value) + ", data=" + dataS
               + ", signatureV=" + (signature == null ? "" : signature.v) + ", signatureR="
